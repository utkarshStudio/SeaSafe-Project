...
import type { Port, Route, Vessel } from "@/lib/types";
...
const MAP_THEME_URLS: Record<MapTheme, string> = {
  ecdis: "/styles/ecdis.json",
  "green-blue": "/styles/green-blue.json",
...
> = {
  ...
  "extreme-weather": { longitude: 87, latitude: 14.5, zoom: 4.0 },
};
...
export function BridgeMap() {
  ...
  const liveShips = useBridgeStore((s) => s.liveShips);

  useEffect(() => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("seasafe-map-theme");
      if (stored && ["ecdis", "green-blue", "satellite", "positron", "dark", "fiord", "liberty", "bright"].includes(stored)) {
      ...
      ...liveShips.map((ship) => {
        if (iconStatus !== "available") {
          return new ScatterplotLayer<Vessel>({
            id: `other-ship-fallback-${ship.id}`,
            data: [ship],
            getPosition: (d) => d.position,
            getRadius: 6,
            radiusUnits: "pixels",
        ...
        return new IconLayer<Vessel>({
          id: `other-ship-icon-${ship.id}`,
          data: [ship],
          getPosition: (d) => d.position,
          getIcon: () => ({
            url: "/icons/vessel.svg",
            width: 128,
            height: 128,
            anchorY: 64,
            anchorX: 64,
    ...
    <div className="absolute inset-0">
      <DeckGL
        ...
        getTooltip={({ object }) => {
          ...
          const v = object as Partial<Vessel>;
          if (v.imo && v.name && v.type) {
            const isMain = v.id === scenario.vessel.id;
            return {
              html: `<div style="font-family:Inter,sans-serif;font-size:11px;max-width:240px;line-height:1.5"><div style="font-weight:700;margin-bottom:4px;color:#f1f5f9">${v.name}</div><div style="color:#94a3b8">${v.type} · IMO ${v.imo}</div><div style="margin-top:6px;color:#22d3ee;text-transform:uppercase;letter-spacing:0.08em;font-size:10px">${isMain ? "Primary Vessel (Active)" : "AIS Target (Sailing Same Route)"}</div></div>`,
              style: {
                background: "rgba(15,23,42,0.95)",
                border: isMain ? "1px solid rgba(6,182,212,0.6)" : "1px solid rgba(71,85,105,0.8)",
                borderRadius: "6px",
                padding: "8px 12px",
                color: "#e2e8f0",
              },
      ...
      >
        <MapGL mapStyle={MAP_THEME_URLS[mapTheme] || MAP_THEME_URLS.dark} reuseMaps />
      </DeckGL>
      ...
      <div className="absolute left-4 top-4 z-10 hidden md:block rounded-md border border-slate-700/70 bg-slate-950/75 px-3 py-2 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <span
            className={`size-1.5 rounded-full ${weather.status === "live"
